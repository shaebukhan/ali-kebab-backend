...
const loginController = async (req, res) => {
    try {
        ...
        if (user.status === 1) {
            return res.status(201).send({
                success: false,
                message: "Account Banned ! Please Contact Customer Support",
            });
        }
        // Generate JWT token
...
const getAlluserController = async (req, res) => {
    try {
        ...
        const totalUsers = await UserModel.countDocuments({
            ...searchQuery,
            role: 0, // Filter only users with role == 0
        });

        // Find users with pagination and search
        const users = await UserModel.find({ ...searchQuery, role: 0 })
            .select("-password")
...
module.exports = {
    registerController, loginController, testController, forgotPasswordController, logoutController, getAllData,
    resetPasswordController,
    getAlluserController, profileUpdateController, addUserAdminController, getSingleUserController, userUpdateController, deleteUserController
};